...
public class AlertController: UIViewController {
    ...
    private func commonInit() {
        ...
        if self.preferredStyle == .actionSheet {
            ...
            Bundle.resourceBundle.loadNibNamed(nibName, owner: self, options: nil)
        }
    }
    ...
    public func add(title: String, style: AlertActionStyle = .normal, handler: (() -> (Void))? = nil) {
        add(AlertAction(title: title, style: style, handler: { (_) in
            handler?()
        }))
    }
    ...
    private func configureAlertView() {
        ...
        if let backgroundImage = self.visualStyle.backgroundImage {
    ...
    private lazy var backroundImageView: UIImageView = {
        ...
        imageView.snp.makeConstraints { (make) in
            ...
            make.bottom.right.equalTo(self.alertView).offset(10)
        }
    ...
    private func createViewConstraints() {
        ...
        switch self.preferredStyle {
            case .actionSheet:
                ...
                let width = min(min(bounds.width, bounds.height), 414) - margins.left - margins.right
                alertView.snp.makeConstraints({ (make) in
                    make.width.equalTo(width * self.visualStyle.width)
                    make.centerX.equalToSuperview()
                    if #available(iOS 11.0, *) {
                        make.bottom.equalTo(view.safeAreaLayoutGuide.snp.bottom).offset(margins.bottom)
                    } else {
                        make.bottom.equalTo(margins.bottom)
                    }
                    make.height.lessThanOrEqualToSuperview().offset(-margins.top)
                })
            case .alert:
                let maximumHeightOffset = -(margins.top + margins.bottom)
                alertView.snp.makeConstraints({ (make) in
                    make.width.equalTo(self.visualStyle.width)
                    make.center.equalToSuperview()
                    make.height.lessThanOrEqualToSuperview().offset(maximumHeightOffset)
                })
