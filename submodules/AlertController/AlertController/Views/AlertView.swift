class AlertView: AlertControllerView {
    ...
    private func createMessageLabelConstraints() {
        ...
        self.pinBottomOfScrollView(to: self.messageLabel, withPriority: UILayoutPriority(rawValue: UILayoutPriority.defaultLow.rawValue + 1))
    }
    ...
    private func createTextFieldsConstraints() {
        ...
        textFieldsView.snp.makeConstraints { (make) in
            make.width.equalTo(self).offset(-widthOffset)
            make.centerX.equalTo(self)
            make.height.equalTo(height)
        }
        self.pinBottomOfScrollView(to: textFieldsView, withPriority: UILayoutPriority(UILayoutPriority.defaultLow.rawValue + 2))
    }
    ...
    private func createCustomContentViewConstraints() {
        ...
        contentView.subviews.forEach { (view) in
            height = max(height, view.frame.maxY)
        }
        contentView.snp.makeConstraints { (make) in
            make.top.equalTo(aligningView!.snp.bottom).offset(topSpacing)
            make.centerX.equalTo(self)
            make.width.equalTo(self).offset(-widthOffset)
            make.height.equalTo(height)
        }
        self.pinBottomOfScrollView(to: self.contentView, withPriority: UILayoutPriority(UILayoutPriority.defaultLow.rawValue + 3))
    }
    ...
    private func createCollectionViewConstraints() {
        ...
        actionsCollectionView.snp.makeConstraints { (make) in
