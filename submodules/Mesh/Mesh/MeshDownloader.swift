...
public typealias MeshDownloaderCompletionHandler = ((_ data: Data?, _ dest: URL?, _ error: NSError?) -> Void)
public typealias MeshDownloaderProgressBlock = ((_ receivedSize: Int64, _ totalSize: Int64) -> Void)

let MeshLoaderEmptyOptionsInfo = [MeshLoaderOptionsInfoItem]()

public let MeshDownloaderErrorDomain = "com.xhb.meshloader.Error"
...
public class MeshDownloader {
    ...
    public init(name: String) {
        ...
        barrierQueue = DispatchQueue(label: "com.xhb.MeshDownloader.Barrier.\(name)", attributes: .concurrent)
        cancelQueue = DispatchQueue(label: "com.xhb.MeshDownloader.Cancel.\(name)")
    }
    ...
    private func setup(progressBlock: MeshDownloaderProgressBlock?, with completionHandler: MeshDownloaderCompletionHandler?, for url: URL, destination: URL?, options: MeshLoaderOptionsInfo, started: @escaping ((FetchLoader) -> Void)) {
        ...
        func prepareFetchLoad() {
            var loadObjectForURL: FetchLoader?
...
func <== (lhs: MeshLoaderOptionsInfoItem, rhs: MeshLoaderOptionsInfoItem) -> Bool {
    ...
    case (.resumeDownload, .resumeDownload): return true
