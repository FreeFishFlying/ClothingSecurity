...
const CGFloat TGPhotoPaintSettingsPadPickerWidth = 300.0f;

@interface MKPhotoPaintSettingsView ()
{
    MKPhotoPaintColorPicker *_colorPicker;
...
@implementation MKPhotoPaintSettingsView
...
- (instancetype)initWithContext:(id<LegacyComponentsContext>)context
{
    ...
    if (self)
    {
        ...
        __weak MKPhotoPaintSettingsView *weakSelf = self;
        _colorPicker = [[MKPhotoPaintColorPicker alloc] init];
        _colorPicker.beganPicking = ^
        {
            __strong MKPhotoPaintSettingsView *strongSelf = weakSelf;
...
- (UIImage *)_imageForIcon:(TGPhotoPaintSettingsViewIcon)icon highlighted:(bool)highlighted
{
    ...
    if (highlighted)
        iconImage = TGTintedImage(iconImage, [MKPhotoEditorInterfaceAssets accentColor]);
