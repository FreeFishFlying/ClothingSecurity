...
@interface MKPainting ()
{
    ...
    __weak MKPaintUndoManager *_undoManager;
    NSUInteger _strokeCount;
}
@end

@implementation MKPainting
...
- (void)paintStroke:(MKPaintPath *)path clearBuffer:(bool)clearBuffer completion:(void (^)(void))completion
{
    [self performAsynchronouslyInContext:^
    {
        ...
        if (status == GL_FRAMEBUFFER_COMPLETE)
        {
            ...
            [self _setupBrush];
            bounds = [MKPaintRender renderPath:path renderState:_renderState];
        }
...
- (void)commitStrokeWithColor:(UIColor *)color erase:(bool)erase
{
    [self performAsynchronouslyInContext:^
    {
        ...
        [self updateWithBlock:^
        {
            ...
            MKPaintShader *shader = erase ? [self shaderForKey:@"compositeWithEraseMask"] : [self shaderForKey:@"compositeWithMask"];
...
- (void)_setupBrush
{
    MKPaintShader *shader = [self shaderForKey:_brush.lightSaber ? @"brushLight" : @"brush"];
    glUseProgram(shader.program);
    
    glActiveTexture(GL_TEXTURE0);
    
    if (_brushTexture == nil)
        _brushTexture = [[MKPaintTexture alloc] initWithCGImage:_brush.stampRef forceRGB:false];
...
{
    MKPaintShader *shader = erase ? [self shaderForKey:@"blitWithEraseMask"] : [self shaderForKey:@"blitWithMask"];
    if (_brush.lightSaber)
...
{
    MKPaintShader *shader = [self shaderForKey:@"blit"];
    glUseProgram(shader.program);
...
- (NSData *)imageDataForRect:(CGRect)rect resultPaintingData:(NSData **)resultPaintingData
{
    ...
    MKPaintShader *blitShader = [self shaderForKey:@"nonPremultipliedBlit"];
...
- (void)registerUndoInRect:(CGRect)rect
{
    ...
    MKPaintSlice *slice = [[MKPaintSlice alloc] initWithData:paintingData bounds:rect];
    NSInteger uuid;
    arc4random_buf(&uuid, sizeof(NSInteger));
    [_undoManager registerUndoWithUUID:uuid block:^(MKPainting *painting, __unused MKPhotoEntitiesContainerView *entitiesContainer, __unused NSInteger uuid)
    {
...
- (void)restoreSlice:(MKPaintSlice *)slice redo:(bool)redo
{
    [self performAsynchronouslyInContext:^
    {
        if (!redo)
...
- (void)_setupShaders
{
    ...
    _shaders = [MKPaintShaderSet setup];
}
