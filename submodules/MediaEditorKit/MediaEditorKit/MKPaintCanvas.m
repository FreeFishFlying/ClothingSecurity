...
@interface MKPaintCanvas () <UIGestureRecognizerDelegate>
{
    MKPaintBuffers *_buffers;
    CGFloat _screenScale;
    CGAffineTransform _canvasTransform;
    CGRect _dirtyRect;
    
    MKPaintInput *_input;
    MKPaintPanGestureRecognizer *_gestureRecognizer;
    bool _beganDrawing;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self != nil)
    {
        ...
        _state = [[MKPaintState alloc] init];
...
- (void)setPainting:(MKPainting *)painting
{
    _painting = painting;
    
    __weak MKPaintCanvas *weakSelf = self;
    painting.contentChanged = ^(CGRect rect)
    {
        __strong MKPaintCanvas *strongSelf = weakSelf;
        if (strongSelf == nil)
            return;
...
{
    _input = [[MKPaintInput alloc] init];
    
    _gestureRecognizer = [[MKPaintPanGestureRecognizer alloc] initWithTarget:self action:@selector(handlePan:)];
...
- (void)handlePan:(MKPaintPanGestureRecognizer *)gestureRecognizer
{
...
- (void)setContext:(EAGLContext *)context
{
    ...
    if (context != nil)
    {
        _buffers = [MKPaintBuffers buffersWithGLContext:context layer:(CAEAGLLayer *)self.layer];
        [_buffers update];
    }
}
