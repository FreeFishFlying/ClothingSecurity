...
@implementation MKPhotoEditorToolButtonsView
{
...
- (void)calculateLandscapeSizeForPossibleButtonTitles:(NSArray *)possibleButtonTitles
{
    ...
    for (NSString *title in possibleButtonTitles)
    {
        ...
        if ([title respondsToSelector:@selector(sizeWithAttributes:)])
            width = CGCeil([title sizeWithAttributes:@{ NSFontAttributeName:[UIFont systemFontOfSize:17] }].width - 1);
        else
            width = CGCeil([title sizeWithFont:[UIFont systemFontOfSize:17]].width - 1);
