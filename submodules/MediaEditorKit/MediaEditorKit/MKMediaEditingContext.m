...
@property (nonatomic, readonly, strong) id<TGMediaEditableItem> item;
@property (nonatomic, readonly, strong) id<MKMediaEditAdjustments> adjustments;

+ (instancetype)adjustmentsUpdateWithItem:(id<TGMediaEditableItem>)item adjustments:(id<MKMediaEditAdjustments>)adjustments;
...
@interface MKMediaEditingContext ()
{
    NSString *_contextId;
...
@implementation MKMediaEditingContext
...
- (instancetype)init
{
    ...
    if (self != nil)
    {
        ...
        NSString *diskCachePath = [documentsPath stringByAppendingPathComponent:[[self class] diskCachePath]];
        _diskCache = [[TGModernCache alloc] initWithPath:diskCachePath size:[[self class] diskMemoryLimit]];
        
        _fullSizeResultsUrl = [NSURL fileURLWithPath:[documentsPath stringByAppendingPathComponent:[NSString stringWithFormat:@"photoeditorresults/%@", _contextId]]];
        ...
        _paintingImagesUrl = [NSURL fileURLWithPath:[documentsPath stringByAppendingPathComponent:[NSString stringWithFormat:@"paintingimages/%@", _contextId]]];
        ...
        _videoPaintingImagesUrl = [NSURL fileURLWithPath:[documentsPath stringByAppendingPathComponent:@"videopaintingimages"]];
        ...
        _paintingDatasUrl = [NSURL fileURLWithPath:[documentsPath stringByAppendingPathComponent:[NSString stringWithFormat:@"paintingdatas/%@", _contextId]]];
...
{
    MKMediaEditingContext *context = [[MKMediaEditingContext alloc] init];
...
- (SSignal *)imageSignalForItem:(NSObject<TGMediaEditableItem> *)item withUpdates:(bool)withUpdates
{
    ...
    NSString *imageUri = [MKMediaEditingContext _imageUriForItemId:itemId];
...
- (SSignal *)thumbnailImageSignalForItem:(id<TGMediaEditableItem>)item withUpdates:(bool)withUpdates synchronous:(bool)synchronous
{
    ...
    NSString *imageUri = [MKMediaEditingContext _thumbnailImageUriForItemId:itemId];
...
- (id<MKMediaEditAdjustments>)adjustmentsForItem:(id<TGMediaEditableItem>)item
{
    NSString *itemId = [self _contextualIdForItemId:item.uniqueIdentifier];
    if (itemId == nil)
        return nil;
...
- (id<MKMediaEditAdjustments>)_adjustmentsForItemId:(NSString *)itemId
{
    if (itemId == nil)
        return nil;
...
- (void)setAdjustments:(id<MKMediaEditAdjustments>)adjustments forItem:(id<TGMediaEditableItem>)item
{
    NSString *itemId = [self _contextualIdForItemId:item.uniqueIdentifier];
    if (itemId == nil)
        return;
    
    id<MKMediaEditAdjustments> previousAdjustments = _adjustments[itemId];
...
- (SSignal *)adjustmentsSignalForItem:(NSObject<TGMediaEditableItem> *)item
{
    ...
    {
        return [update.item.uniqueIdentifier isEqualToString:item.uniqueIdentifier];
    }] map:^id<MKMediaEditAdjustments>(TGMediaAdjustmentsUpdate *update)
    {
...
{
    NSString *itemId = [self _contextualIdForItemId:item.uniqueIdentifier];
    id<MKMediaEditAdjustments> adjustments = [self adjustmentsForItem:item];
