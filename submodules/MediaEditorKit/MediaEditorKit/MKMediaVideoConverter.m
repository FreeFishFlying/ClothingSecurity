...
@interface MKMediaVideoConversionPresetSettings ()
...
@interface MKMediaSampleBufferProcessor : NSObject
{
...
@interface MKMediaVideoFileWatcher ()
{
...
@interface MKMediaVideoConversionContext : NSObject
...
@property (nonatomic, readonly) MKMediaSampleBufferProcessor *videoProcessor;
@property (nonatomic, readonly) MKMediaSampleBufferProcessor *audioProcessor;
...
- (instancetype)contextWithAssetReader:(AVAssetReader *)assetReader assetWriter:(AVAssetWriter *)assetWriter videoProcessor:(MKMediaSampleBufferProcessor *)videoProcessor audioProcessor:(MKMediaSampleBufferProcessor *)audioProcessor timeRange:(CMTimeRange)timeRange dimensions:(CGSize)dimensions;

@end


@interface MKMediaVideoConversionResult ()
...
@implementation MKMediaVideoConverter
...
+ (SSignal *)convertAVAsset:(AVAsset *)avAsset adjustments:(TGMediaVideoEditAdjustments *)adjustments watcher:(MKMediaVideoFileWatcher *)watcher inhibitAudio:(bool)inhibitAudio
{
    ...
    {
        SAtomic *context = [[SAtomic alloc] initWithValue:[MKMediaVideoConversionContext contextWithQueue:queue subscriber:subscriber]];
        ...
        [avAsset loadValuesAsynchronouslyForKeys:requiredKeys completionHandler:^
        {
            [queue dispatch:^
            {
                if (((MKMediaVideoConversionContext *)context.value).cancelled)
                    return;
                ...
                {
                    MKMediaVideoConversionContext *resultContext = context.value;
                    [resultContext.imageGenerator generateCGImagesAsynchronouslyForTimes:@[ [NSValue valueWithCMTime:kCMTimeZero] ] completionHandler:^(__unused CMTime requestedTime, CGImageRef  _Nullable image, __unused CMTime actualTime, AVAssetImageGeneratorResult result, __unused NSError * _Nullable error)
                    {
                        ...
                        __block MKMediaVideoConversionResult *contextResult = nil;
                        [context modify:^id(MKMediaVideoConversionContext *resultContext)
                        {
                            id liveUploadData = nil;
                            if (watcher != nil)
                                liveUploadData = [watcher fileUpdated:true];
                            
                            contextResult = [MKMediaVideoConversionResult resultWithFileURL:outputUrl fileSize:0 duration:CMTimeGetSeconds(resultContext.timeRange.duration) dimensions:resultContext.dimensions coverImage:coverImage liveUploadData:liveUploadData];
        ...
        return [[SBlockDisposable alloc] initWithBlock:^
        {
            [queue dispatch:^
            {
                [context modify:^id(MKMediaVideoConversionContext *currentContext)
                {
...
+ (CGSize)dimensionsFor:(CGSize)dimensions adjustments:(TGMediaVideoEditAdjustments *)adjustments preset:(TGMediaVideoConversionPreset)preset {
    ...
    CGSize maxDimensions = [MKMediaVideoConversionPresetSettings maximumSizeForPreset:preset];
...
+ (AVAssetReaderVideoCompositionOutput *)setupVideoCompositionOutputWithAVAsset:(AVAsset *)avAsset composition:(AVMutableComposition *)composition videoTrack:(AVAssetTrack *)videoTrack preset:(TGMediaVideoConversionPreset)preset adjustments:(TGMediaVideoEditAdjustments *)adjustments timeRange:(CMTimeRange)timeRange outputSettings:(NSDictionary **)outputSettings dimensions:(CGSize *)dimensions conversionContext:(SAtomic *)conversionContext
{
    ...
    [conversionContext modify:^id(MKMediaVideoConversionContext *context)
    {
        return [context addImageGenerator:imageGenerator];
    }];
    
    *outputSettings = [MKMediaVideoConversionPresetSettings videoSettingsForPreset:preset dimensions:outputDimensions];
...
{
    MKMediaSampleBufferProcessor *videoProcessor = nil;
    MKMediaSampleBufferProcessor *audioProcessor = nil;
    ...
    videoProcessor = [[MKMediaSampleBufferProcessor alloc] initWithAssetReaderOutput:output assetWriterInput:input];
    
    if (!inhibitAudio && [MKMediaVideoConversionPresetSettings keepAudioForPreset:preset] && audioTrack != nil)
    {
        ...
        AVAssetWriterInput *input = [AVAssetWriterInput assetWriterInputWithMediaType:AVMediaTypeAudio outputSettings:[MKMediaVideoConversionPresetSettings audioSettingsForPreset:preset]];
        if ([assetWriter canAddInput:input]) {
            [assetWriter addInput:input];
        }
        
        audioProcessor = [[MKMediaSampleBufferProcessor alloc] initWithAssetReaderOutput:output assetWriterInput:input];
    }
    
    [outConversionContext modify:^id(MKMediaVideoConversionContext *currentContext)
    {
...
{
    MKMediaVideoConversionContext *context = [context_ value];
...
{
    NSInteger bitrate = [MKMediaVideoConversionPresetSettings _videoBitrateKbpsForPreset:preset];
    if (hasAudio)
        bitrate += [MKMediaVideoConversionPresetSettings _audioBitrateKbpsForPreset:preset] * [MKMediaVideoConversionPresetSettings _audioChannelsCountForPreset:preset];
...
+ (TGMediaVideoConversionPreset)bestAvailablePresetForDimensions:(CGSize)dimensions
{
    ...
    {
        CGFloat presetMaxSide = [MKMediaVideoConversionPresetSettings maximumSizeForPreset:(TGMediaVideoConversionPreset)i].width;
...
@implementation MKMediaSampleBufferProcessor
...
@implementation MKMediaVideoFileWatcher
...
- (dispatch_source_t)_setup
{
    ...
    if (fd > 0)
    {
        ...
        __weak MKMediaVideoFileWatcher *weakSelf = self;
        dispatch_source_set_event_handler(source, ^
        {
            __strong MKMediaVideoFileWatcher *strongSelf = weakSelf;
            if (strongSelf == nil)
                return;
...
@implementation MKMediaVideoConversionContext

+ (instancetype)contextWithQueue:(SQueue *)queue subscriber:(SSubscriber *)subscriber
{
    MKMediaVideoConversionContext *context = [[MKMediaVideoConversionContext alloc] init];
...
@implementation MKMediaVideoConversionResult
...
{
    MKMediaVideoConversionResult *result = [[MKMediaVideoConversionResult alloc] init];
...
@implementation MKMediaVideoConversionPresetSettings
