...
static NSBundle *frameworkBundle() {
    ...
    dispatch_once(&onceToken, ^{
        ...
        NSString *updatedPath = [[currentBundle bundlePath] stringByAppendingPathComponent:@"MediaEditorKit.bundle"];
...
UIImage *TGComponentsImageNamed(NSString *name) {
    if (iosMajorVersion() < 8)
        return [UIImage imageNamed:[NSString stringWithFormat:@"MediaEditorKit.bundle/%@", name]];
