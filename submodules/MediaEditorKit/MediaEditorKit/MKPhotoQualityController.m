...
@interface TGPhotoQuality : NSObject <MKPhotoEditorItem>
...
@interface MKPhotoQualityController ()
{
...
@implementation MKPhotoQualityController
...
- (void)loadView
{
    ...
    __weak MKPhotoQualityController *weakSelf = self;
    void(^interactionEnded)(void) = ^
    {
        __strong MKPhotoQualityController *strongSelf = weakSelf;
        if (strongSelf == nil)
            return;
    ...
    if (!CGSizeEqualToSize(dimensions, CGSizeZero))
        _quality.maximumValue = [MKMediaVideoConverter bestAvailablePresetForDimensions:dimensions] - 1;
    else
...
- (void)_animatePreviewViewTransitionOutToFrame:(CGRect)targetFrame saving:(bool)saving parentView:(UIView *)__unused parentView completion:(void (^)(void))completion
{
    ...
    if (saving && CGRectIsNull(targetFrame) && parentView != nil)
    {
        ...
        snapshotAlphaAnimation = [MKPhotoEditorAnimation prepareTransitionAnimationForPropertyNamed:kPOPViewAlpha];
...
{
    CGRect containerFrame = [MKPhotoQualityController photoContainerFrameForParentViewFrame:self.view.frame toolbarLandscapeSize:self.toolbarLandscapeSize orientation:orientation panelSize:TGPhotoEditorQualityPanelSize];
...
- (CGRect)_targetFrameForTransitionInFromFrame:(CGRect)fromFrame
{
    ...
    CGRect containerFrame = [MKPhotoQualityController photoContainerFrameForParentViewFrame:CGRectMake(0, 0, referenceSize.width, referenceSize.height) toolbarLandscapeSize:self.toolbarLandscapeSize orientation:orientation panelSize:TGPhotoEditorQualityPanelSize];
...
- (void)updateLayout:(UIInterfaceOrientation)orientation
{
    ...
    CGRect containerFrame = [MKPhotoEditorTabController photoContainerFrameForParentViewFrame:CGRectMake(0, 0, referenceSize.width, referenceSize.height) toolbarLandscapeSize:self.toolbarLandscapeSize orientation:orientation panelSize:panelSize];
...
- (void)_updateVideoDuration:(NSTimeInterval)duration hasAudio:(bool)hasAudio
{
    ...
    MKVideoEditAdjustments *adjustments = [self.photoEditor exportAdjustments];
...
- (void)updateInfo
{
    NSUInteger estimatedSize = [MKMediaVideoConverter estimatedSizeForPreset:self.preset duration:_fileDuration hasAudio:_hasAudio];
    
    CGRect cropRect = _photoEditor.cropRect;
    CGSize maxDimensions = [MKMediaVideoConversionPresetSettings maximumSizeForPreset:self.preset];
    ...
    outputDimensions = [MKMediaVideoConverter _renderSizeWithCropSize:outputDimensions];
    ...
    [(MKPhotoEditorController *)self.parentViewController setInfoString:fileSize];
}
...
- (void)generateVideoPreview
{
    ...
    }] mapToSignal:^SSignal *(AVAsset *avAsset)
    {
        ...
        {
            return [MKMediaVideoConverter convertAVAsset:avAsset adjustments:adjustments watcher:nil inhibitAudio:true];
    ...
    else
    {
        ...
        {
            if ([value isKindOfClass:[MKMediaVideoConversionResult class]])
            {
                MKMediaVideoConversionResult *result = (MKMediaVideoConversionResult *)value;
...
- (UIView <MKPhotoEditorToolView> *)itemControlViewWithChangeBlock:(void (^)(id newValue, bool animated))changeBlock
{
    ...
    UIView <MKPhotoEditorToolView> *view = [[MKPhotoEditorGenericToolView alloc] initWithEditorItem:self];
...
- (UIView<MKPhotoEditorToolView> *)itemAreaViewWithChangeBlock:(void (^)(id))__unused changeBlock
{
    return nil;
}
