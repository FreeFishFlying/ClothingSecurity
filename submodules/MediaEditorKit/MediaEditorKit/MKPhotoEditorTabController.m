...
@implementation MKPhotoEditorTabController

- (void)handleTabAction:(MKPhotoEditorTab)__unused tab
{
}
...
{
    CGRect containerFrame = [MKPhotoEditorTabController photoContainerFrameForParentViewFrame:self.view.frame toolbarLandscapeSize:self.toolbarLandscapeSize orientation:orientation panelSize:TGPhotoEditorPanelSize];
...
- (bool)inFormSheet
{
    return [(MKViewController *)[self parentViewController] inFormSheet];
}
...
{
    MKPhotoEditorTab highlightedButtons = MKPhotoEditorNoneTab;
    
    if ([editorValues cropAppliedForAvatar:forAvatar])
        highlightedButtons |= MKPhotoEditorCropTab;
    
    if ([editorValues hasPainting])
        highlightedButtons |= MKPhotoEditorPaintTab;
