...
@interface MKPhotoCropView () <UIScrollViewDelegate>
{
    ...
    MKPhotoCropAreaView *_areaView;
    CGRect _previousAreaFrame;
    
    MKPhotoCropScrollView *_scrollView;
    ...
    MKPhotoCropRotationView *_rotationView;
    
    UIInterfaceOrientation _previousInterfaceOrientation;
}
@end

@implementation MKPhotoCropView
...
- (instancetype)initWithOriginalSize:(CGSize)originalSize hasArbitraryRotation:(bool)hasArbitraryRotation
{
    ...
    if (self != nil)
    {
        ...
        __weak MKPhotoCropView *weakSelf = self;
        ...
        _scrollView = [[MKPhotoCropScrollView alloc] initWithFrame:CGRectZero];
        _scrollView.shouldBeginChanging = ^bool
        {
            __strong MKPhotoCropView *strongSelf = weakSelf;
            if (strongSelf == nil)
                return true;
        ...
        UIColor *overlayColor = iosMajorVersion() >= 9 ? UIColorRGBA(0x000000, 0.45f) : [MKPhotoEditorInterfaceAssets cropTransparentOverlayColor];
        ...
        _areaView = [[MKPhotoCropAreaView alloc] initWithFrame:self.bounds];
        ...
        {
            _rotationView = [[MKPhotoCropRotationView alloc] initWithFrame:CGRectZero];
...
- (void)startConfirmCountdown
{
    ...
    _confirmTimer = [[STimer alloc] initWithTimeout:0.2f repeat:false completion:^
    {
        __strong MKPhotoCropView *strongSelf = weakSelf;
        if (strongSelf == nil)
            return;
...
- (void)rotate90DegreesCCWAnimated:(bool)animated
{
    ...
    _areaWrapperView.backgroundColor = [MKPhotoEditorInterfaceAssets toolbarBackgroundColor];
