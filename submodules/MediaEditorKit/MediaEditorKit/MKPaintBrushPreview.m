...
@interface MKPaintBrushPreview ()
{
    EAGLContext *_context;
    
    MKPaintBrush *_brush;
    MKPaintShader *_brushShader;
    MKPaintShader *_brushLightShader;
    MKPaintShader *_blitLightShader;
    MKPaintTexture *_brushTexture;
...
@implementation MKPaintBrushPreview
...
- (instancetype)init
{
    ...
    if (self != nil)
    {
        ...
        NSDictionary *availableShaders = [MKPaintShaderSet availableShaders];

        NSDictionary *shader = availableShaders[@"brush"];
        _brushShader = [[MKPaintShader alloc] initWithVertexShader:shader[@"vertex"] fragmentShader:shader[@"fragment"] attributes:shader[@"attributes"] uniforms:shader[@"uniforms"]];
        
        shader = availableShaders[@"brushLight"];
        _brushLightShader = [[MKPaintShader alloc] initWithVertexShader:shader[@"vertex"] fragmentShader:shader[@"fragment"] attributes:shader[@"attributes"] uniforms:shader[@"uniforms"]];
        
        shader = availableShaders[@"brushLightPreview"];
        _blitLightShader = [[MKPaintShader alloc] initWithVertexShader:shader[@"vertex"] fragmentShader:shader[@"fragment"] attributes:shader[@"attributes"] uniforms:shader[@"uniforms"]];
...
- (void)_generateBrushPath
{
    ...
    for (NSUInteger i = 0; i < TGPaintBrushPreviewSegmentsCount; i++)
    {
        ...
        MKPaintPoint *point = [MKPaintPoint pointWithX:pt.x y:pt.y z:fraction];
    ...
    _path = [[MKPaintPath alloc] initWithPoints:points];
...
- (void)_setupBrush
{
    MKPaintShader *shader = _brush.lightSaber ? _brushLightShader : _brushShader;
    glUseProgram(shader.program);
    glActiveTexture(GL_TEXTURE0);
    
    if (_brushTexture == nil)
        _brushTexture = [[MKPaintTexture alloc] initWithCGImage:_brush.previewStampRef forceRGB:false];
...
- (UIImage *)imageForBrush:(MKPaintBrush *)brush size:(CGSize)size
{
    ...
    [MKPaintRender renderPath:_path renderState:_renderState];
    ...
    if (_brush.lightSaber)
    {
        ...
        MKPaintShader *shader = _blitLightShader;
        glUseProgram(shader.program);
